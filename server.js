...
//app.post : ajouter contact  document to collection in database
...
// retourne tous contact dans collection
...
// retourne one contact by id dans collection
...
// modifier contact exist by id
app.put('/modify_contact/:id' , (req, res) => {
    let ContactToEditId = ObjectID(req.params.id)
    let modifiedContact = req.body
    db.collection('contacts').findOneAndUpdate({_id : ContactToEditId}, {$set:{...modifiedContact}}, (err, data) => {
